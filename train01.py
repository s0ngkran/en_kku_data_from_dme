...
# load data json
with open('training_json_with_covered', 'r') as f:
    training_json = json.load(f)
with open('validation_json_with_covered', 'r') as f:
    validation_json = json.load(f)

# load data
training_set = DMEDataset(training_json, test_mode=False)
validation_set = DMEDataset(validation_json, test_mode=False)
training_set_loader = DataLoader(training_set, batch_size=BATCH_SIZE, num_workers=5)
validation_set_loader = DataLoader(validation_set,  batch_size=BATCH_SIZE, num_workers=5)
