...
if __name__ == '__main__':
    ############################ config ###################
    TRAINING_JSON = './training.4keypoint.no_curriculum.json'
    VALIDATION_JSON = './validation.4keypoint.no_curriculum.json'
    BATCH_SIZE = 20
    SAVE_EVERY = 2
    LEARNING_RATE = 1e-4
    ...
    CONTINUE_PATH = './save/train11.pyepoch0000003702.model'
    NEW_LEARNING_RATE = 1e-4

    # check result
    IS_CHECK_RESULT = True
    TESTING_JSON = './training.4keypoint.no_curriculum.json'
    DEVICE = 'cpu'
    TESTING_FOLDER = 'TESTING_FOLDER'
    WEIGHT_PATH = './save/train11.pyepoch0000000322.model'
    ############################################################
    print('starting...')
            ...
            for iteration, dat in enumerate(testing_set_loader):
                ...
                for i, gtl in enumerate(dat['gtl']):
                    for ii, img in enumerate(gtl):
                        ...
                        # cv2.imwrite(os.path.join(TESTING_FOLDER, str(iteration)+'temp_gtl_%d_%d.jpg'%(i, ii)), img)
